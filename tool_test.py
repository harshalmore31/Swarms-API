...
def run_single_swarm():
    ...
    response = requests.post(
        f"{BASE_URL}/v1/swarm/completions", headers=headers, json=initial_payload
    )
    ...
    try:
        ...
        if function_args and "location" in function_args:
            ...
            if followup_response.status_code == 200:
                ...
                if "output" in final_result and isinstance(
                    final_result["output"], list
                ):
                    for message in final_result["output"]:
                        if message.get("role") == "AI assistant" and message.get(
                            "content"
                        ):
                            content = message.get("content")
                            if "Time:" in content and "\n" in content:
